	...
	public class WIPController : MonoBehaviour
	{
        ...
        /*void CheckForUpCalibration()
        {
            if (calibrationText != null)
            {
                ...
                {
                    if (hand.GetStandardSecondaryButtonDown())//calibrate button pressed
                    {
                        Calibrator.Instance.naturalUp = StepDetector.instance.hmd.up;
                        Calibrator.Instance.playerHeight = forwardTransform.position.y;

                        calibrationText.text = "Calibration confirmed";
		...
		void CheckJump()
		{
            ...
            float upwardsAccel = StepDetector.instance.diff;//Vector3.Dot(Input.gyro.userAcceleration, up);
